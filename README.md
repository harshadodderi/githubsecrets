---
title: "You Made a Private Repo Public by Mistake — Now What? A Complete GitHub Secrets Guide"
subtitle: "From incident response to pre-commit hooks — every layer of defense explained."
seo_title: "GitHub Secrets Exposed: Incident Response & Prevention Guide (2025)"
seo_description: "Learn exactly what to do when a GitHub secret leaks — and how to prevent it with pre-commit hooks, OIDC, push protection, and git history cleanup."
tags: github, security, devops, githubactions, devsecops, git, webdev, programming
cover_image_prompt: "A dark terminal window showing a `git log` output with redacted credential strings, overlaid with subtle red warning indicators. Minimalist, clean, cybersecurity aesthetic."
---

> **🚨 Already in an incident?** Jump to [Incident Response](#incident-response) right now. Come back and read the rest after you've rotated everything.
...
> **Note:** Run `git-filter-repo` on a **fresh regular clone** of the repo, not the mirror backup. The mirror is your safety net only.

```bash
# Fresh regular clone to work in
...
**Clean up orphaned objects, then re-add the remote and force-push:**

```bash
# Remove dangling objects left behind by the rewrite
...
resource "aws_iam_openid_connect_provider" "github" {
  ...
  # Thumbprint pinning is no longer enforced for this provider — the value below is
  # accepted by the API but AWS will not reject tokens based on it.
...
> **Note:** Workflows from fork PRs run in a fully isolated context — org-level secrets and environment secrets are never passed, regardless of configuration. This is by design and cannot be overridden from the workflow side.
...
| Using `workflow_dispatch` inputs to pass secrets | Inputs are visible in the Actions run UI and logs to anyone with Actions read access on the repository. Use GitHub Secrets instead — they are never reflected in logs. |
